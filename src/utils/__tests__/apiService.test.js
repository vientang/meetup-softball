...
describe('Update player stats', () => {
    it('merge current player stats with career stats', () => {
        expect(updateMergedPlayerStats(
            existingPlayerStats, 
    ...
    it('merges existing player stats with stats from current game', () => {
        ...
        expect(mergeAndSavePlayerStats(
            existingPlayerStats,
            currentPlayers
        )).toEqual(mergedResults);
    });
    ...
    it('add derived stats', () => {
        expect(addDerivedStats(
            [currentPlayerStats[0]],
            true
        )).toEqual([{ ...currentPlayerStats[0], w: "1", l: "0", gp: "1" }]);

        expect(addDerivedStats(
            [currentPlayerStats[1]],
            false
        )).toEqual([{ ...currentPlayerStats[1], w: "0", l: "1", gp: "1" }]);
    });
});
